...
const TableLocationAdmin = () => {
  ...
  const handleUpdate = (item: any) => {
    dispatch(
      changeComponent(<ModalAddLocation type="editLocation" item={item} />)
    );
    dispatch(setIsOpen(true));
  };
  ...
  const handleSearch = () => {
    ...
    const locationFilterSearch = location.filter((item: LocationDetail) => {
      ...
      const name: string = removeVietnameseTones(
        item.tenViTri.toLocaleLowerCase()
      );
      ...
      return (
        ...
        name === inputSearchLowercase ||
        province === inputSearchLowercase ||
        country === inputSearchLowercase
      );
    });
  ...
  return (
    <div className="table__user__admin">
      ...
      <div className="pagination">
        <Pagination
          totalPages={Number(
            (inputSearch.current.length === 0 ? location : locationFilterSearch)
              .length
          )}
