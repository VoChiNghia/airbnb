...
const Cart = ({ data }: Props) => {
  ...
  const handleClick = () => {
    if (user) {
      ...
      const getSaveData: SaveDataModal[] = getStoreJson("saveData")
        ? getStoreJson("saveData")
        : [];

      getSaveData.forEach((saveData: SaveDataModal) => {
        saveData.maPhong === data.id
          ? setActiveHeart(true)
    ...
    <div className="card">
      <NavLink to={`/detail/${data.id}`}>
        <div className="card-img">
          ...
          >
            <SwiperSlide>
              <img src={data?.hinhAnh} alt="" width="100%" />
            </SwiperSlide>
            <SwiperSlide>
              <img src={data?.hinhAnh} alt="" width="100%" />
            </SwiperSlide>
        ...
        <div className="card__info">
          <p className="name">
            {data.tenPhong.length > 32
              ? data.tenPhong.slice(0, 33)
              : data.tenPhong}
          </p>
          <p className="star">
            <span>
        ...
        <p className="description">
          {data.moTa.length > 35 ? data.moTa.slice(0, 35) + "..." : data.moTa}
        </p>
      ...
      <div className="heart" onClick={handleClick}>
        {activeHeart ? (
          <AiFillHeart className="heart-icon-fill" />
        ) : (
          <svg
            viewBox="0 0 32 32"
