...
const locationReducer = createSlice({
  ...
  reducers: {
    getLocation: (
      state: LocationState,
      action: PayloadAction<LocationDetail[]>
    ) => {
...
export const { getLocation, longAndLat, addresReducer } =
      ...
      res.data.content.map(async (data: LocationDetail, index: number) => {
        const response = await http.get(
          `https://api.mapbox.com/geocoding/v5/mapbox.places/${
            data.tenViTri + " " + data.tinhThanh
          }.json?access_token=pk.eyJ1Ijoidm9jaGluZ2hpYSIsImEiOiJjbGNwMm1sN2IwMG52M3dvZG41b21kMGN4In0.P43VVScuPtGFkFt7fIrVDQ`
        );
